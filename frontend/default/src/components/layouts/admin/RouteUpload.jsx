...
const RouteUpload = () => {
    ...
    const [routeDetails, setRouteDetails] = useState(state.routePlan[routeIndex] || 
        {
        day : "",
        routeTask : "",
        routeMapCoordinates : {
    ...
    const mutation = useMutation({
        mutationFn : async()=> {
             await destinationService.addRoutePlan(state._id, routes.current)
        },
        onSuccess : () => {
                ...
                {routes.current[0] && routes.current.map((route, index) => {
                    return (
                     ...
                     {routes.current[0] && <Button variant="secondary" size='sm' children="Next" onClick={()=>{
                        ...
                        <Button 
                  onClick={() =>  state  ? navigate(`/admin/upload/${state._id}/photoUpload`) : alert("create destination first!!")} 
                  children="Add Photos" 
                  size='md'
                  variant='secondary'
                  />  
