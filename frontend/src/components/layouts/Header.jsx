          ...
          >Signup</button>
            ...
            <div className='flex gap-6 items-center'>
              ...
              <div className=''>
                ...
                <img alt='' className='w-8 h-8 rounded-full' src={state.userData.avatar}/>
                :
                <FontAwesomeIcon onClick={() => navigate("/userProfile")} className={`cursor-pointer ${pathname === "/" ? "text-white" : "text-black"} h-6`} icon={faCircleUser} />
              }   
              </div>  
            </div>

          )}
          <ul className={`${!setting && "hidden"}  w-[15%] text-center  bg-gray-200 absolute top-16 right-12`} >
          <li className='mb-2 cursor-pointer hover:bg-gray-300 p-2'  onClick ={() => {setSetting(false),navigate("/settings")}} >
            Setting
          </li>
          <li className=' cursor-pointer hover:bg-gray-300 p-2' onClick ={() => {setSetting(false),handleLogout()}} >
            Logout
          </li>
          </ul>
